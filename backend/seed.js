...
const sampleQAMessages = [
  {
    sender: 'ai',
    text: 'Hello! I am your HealthLens AI Document Assistant powered by MongoDB on Render.',
    timestamp: 'Just now'
  }
];

export const seedDatabase = async () => {
  if (process.env.DATABASE_MODE !== 'mongodb' || !MONGODB_URI) {
    console.log('Database seeding skipped: DATABASE_MODE is not mongodb.');
    return;
  }
  ...
  try {
    ...
    if (recordCount === 0) {
      ...
      console.log(`Database contains ${recordCount} records. Ready.`);
    }
  } catch (err) {
